...
public class ChainedSSLSocketFactory extends SSLSocketFactory {
    ...
    public ChainedSSLSocketFactory(KeyStore truststore, SSLSocketFactory defaultFactory) throws NoSuchAlgorithmException, KeyStoreException, KeyManagementException, UnrecoverableKeyException {
        super(truststore);
        if (defaultFactory != null) {
            mDefaultFactory = defaultFactory;
        } else {
            mDefaultFactory = SSLSocketFactory.getSocketFactory();
        }
    }
    ...
    public Socket createSocket(Socket s, String host, int port, boolean autoClose) throws IOException, UnknownHostException {
        ...
        if (this.mDefaultFactory != null) {
            try {
                socket = mDefaultFactory.createSocket(s, host, port, autoClose);
    ...
    public Socket createSocket() throws IOException {
        ...
        if (this.mDefaultFactory != null) {
            try {
                socket = mDefaultFactory.createSocket();
            } catch (IOException ioe) {
                exception = ioe;
            }
        }
